<div>
    ...
    <div class="relative">
        <div class="grid grid-cols-1 md:grid-cols-2 items-center mb-2 gap-y-3">
            <div class="flex w-full items-center h-10">
                @if($this->searchableColumns()->count())
                    <div class="flex w-full md:max-w-sm rounded-lg">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <input wire:model.debounce.500ms="search" class="pr-8 bg-gray-200 block w-full py-3 pl-10 text-sm border-gray-300 leading-4 rounded-md focus:outline-none" placeholder="{{__('Search in')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
                            <div class="absolute inset-y-0 right-0 flex items-center pr-2">
                                <button wire:click="$set('search', null)" class="text-gray-400 hover:text-red-600 focus:outline-none transition-all">
            ...
            <div class="flex flex-wrap items-center space-x-1 w-full justify-center md:justify-end">
                ...
                @if(count($this->massActionsOptions))
                    <div class="flex items-center justify-center space-x-1">
                        ...
                        <select wire:model="massActionOption" class="px-3 text-xs font-medium tracking-wider uppercase bg-gray-200 py-2 space-x-2 rounded-md leading-4 focus:outline-none" id="datatables_mass_actions">
                        ...
                        <button
                            ...
                            class="flex items-center px-4 py-2 text-xs font-medium tracking-wider uppercase bg-gray-200 border border-gray-300 rounded-md leading-4 hover:bg-gray-100 transition focus:outline-none" type="submit" title="Submit"
                        >Go</button>
                    </div>
                @endif
        ...
        <div wire:loading.class="opacity-50" class="rounded-lg @unless($complex || $this->hidePagination) rounded-b-none @endunless bg-white max-w-screen overflow-x-scroll border-2 @if($this->activeFilters) border-blue-500 @else border-transparent @endif @if($complex) rounded-b-none border-b-0 @endif">
            <div>
